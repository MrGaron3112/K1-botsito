...
if(comando.name) { client.commands.set(comando.name, comando)
        comandos++         
                 } else {
  console.log(`el comando [/${carpeta}/${archivo}] no esta configurado`.brightRed)
continue;
            }
  if (comando.aliases && Array.isArray(comando.aliases)) comando.aliases.forEach((alias) => client.aliases.set(alias, comando.name))
}
})
    console.log(`${comandos} comandos cargados correctamente`.green)
  }catch(e){
console.log(e)
}
}